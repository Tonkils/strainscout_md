...
export default function FlowerCard({ strain }: FlowerCardProps) {
  ...
  return (
    <Link href={`/strain/${strain.id}`}>
      <div className="group relative bg-card border border-border/50 rounded-lg overflow-hidden hover:border-emerald-500/30 transition-all duration-300 hover:shadow-lg hover:shadow-emerald-500/5">
        {/* Category Badge */}
        <div className="absolute top-3 right-3 flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-400 text-[9px] font-semibold uppercase tracking-wider">
        ...
        <div className="p-5">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground mb-2">
            {strain.genetics && (
              <span className="truncate max-w-[160px]" title={strain.genetics}>
                {strain.genetics}
              </span>
            )}
          </div>
