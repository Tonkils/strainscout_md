...
export default function EdibleCard({ strain }: EdibleCardProps) {
  ...
  return (
    <Link href={`/strain/${strain.id}`}>
      <div className="group relative bg-card border border-border/50 rounded-lg overflow-hidden hover:border-pink-500/30 transition-all duration-300 hover:shadow-lg hover:shadow-pink-500/5">
        {/* Category Badge */}
        <div className="absolute top-3 right-3 flex items-center gap-1 px-2 py-0.5 rounded-full bg-pink-500/15 text-pink-400 text-[9px] font-semibold uppercase tracking-wider">
        ...
        <div className="p-5">
          ...
          {/* Product Name */}
          <h3 className="font-serif text-lg text-foreground group-hover:text-pink-400 transition-colors mb-1 line-clamp-2">
            {strain.name}
          </h3>

          {/* THC (mg/serving) / CBD / Quantity */}
